...
/**
 * Read-only "flow" view of a session's execution, derived entirely from the
 * ChatBlock tree — no new data capture. Two panels:
 *   - Files touched: which files this session read/edited, ranked by activity.
 *   - Execution flow: the tool-call / subagent topology as an indented graph.
 *
 * This is the desktop (2D) surface of the shared flow substrate; the same
 * aggregates feed the fleet contention signal and the XR garden later.
 */
...
export function SessionFlowPanel(props: {
    ...
    const touches = useMemo(() => collectFileTouches(props.blocks), [props.blocks])
    ...
    return (
        <div className="mx-auto w-full max-w-content px-3 pb-3">
            <div className="rounded-xl border border-[var(--app-border)] bg-[var(--app-subtle-bg)] p-3">
                ...
                {touches.length === 0 ? (
                    <div className="text-sm text-[var(--app-hint)]">{t('session.flow.filesEmpty')}</div>
                ) : (
