...
- **NEVER merge on `tiann/hapi`.** Only @tiann merges upstream `main`. Do not run `gh pr merge` (or enable auto-merge) on upstream PRs — even with write access, even when a handoff says “babysit to merge.” Prepare only; see `docs/operator/AGENTS.md` § Upstream relationship.
