...
(function () {
  ...
  /**
   * #364/#375 — Quest + sttUrl must keep MediaRecorder so whisper can run on Send.
   * Web Speech is still started afterwards, for the live label only, and dropped
   * if it kills the recorder tracks.
   */
  ...
  function makeRecognition(onFinal, onInterim) {
    ...
    r.onerror = function (ev) {
      ...
      if (!msg) return;
      // #375: Quest speech recognition is a live label. An SR error must not
      // abort a recorder that whisper still needs.
      if (preferWhisperCapture() && recording) {
        yieldSpeechKeepRecorder();
        return;
      }
      toast(msg, 'err');
  ...
  function openOverlay(shotDataUrl) {
    ...
    // #373: no toolbar Undo / Clear / Send. Clear only wiped strokes and looked
    // like cancel, so operators never reached H. Voice-capable execute: mic
    // (while recording) is Send, H dismisses the overlay. Swatches stay.
    // Text-first finishes from the composer — that path has no mic-as-Send.
  ...
  function mediaTracksLive() {
    if (!mediaStream || typeof mediaStream.getTracks !== 'function') return false;
  ...
  var questSpeechYielded = false;

  /** #375: drop the live-label recognizer. Restart MediaRecorder only if it lost the mic. */
  function yieldSpeechKeepRecorder() {
    if (questSpeechYielded) return;
    questSpeechYielded = true;
    stopWebRecognition();
    if (!recording) return;
    if (mediaTracksLive()) {
      refreshRecordLabel();
    ...
    startMediaCapture().then(function (ok) {
      if (!recording) return;
      if (ok) refreshRecordLabel();
    });
  }

  /**
   * #375: recorder is already running. Start Web Speech for the live label.
   * If that ends the tracks, stop speech recognition and keep MediaRecorder.
   */
  function attachQuestLiveLabel() {
    questSpeechYielded = false;
    if (!mediaStream || typeof mediaStream.getTracks !== 'function') return false;
    var tracks = mediaStream.getTracks();
    for (var i = 0; i < tracks.length; i++) {
      (function (track) {
        if (!track || track._opdockEndedWatch) return;
        track._opdockEndedWatch = true;
  ...
  function startWebStt() {
    recognition = makeRecognition(
      ...
      function (txt) {
        liveInterim = txt || '';
        if (liveInterim) liveFromSpeech = true;
  ...
  function startRecording(providedShot) {
    ...
    function armed(shot) {
      ...
      // #375: Quest starts the recorder first, shows a listening label immediately,
      // then tries Web Speech for live words. If that kills the tracks, SR is dropped
      // and MediaRecorder is kept for whisper. Desktop stays startWebStt then recorder.
      if (preferWhisperCapture()) {
        refreshRecordLabel();
        startMediaCapture().then(function (mediaOk) {
          if (!recording) return;
          if (!mediaOk) {
            var questHint = (!window.isSecureContext && cfg && cfg.sttUrl)
  ...
  function finishRecording() {
    ...
    // #375: interim words are real speech. Send them instead of waiting for whisper.
    if (!transcript && liveFromSpeech && liveInterim) transcript = String(liveInterim).trim();
  ...
  function loadBundledChangelog() {
    ...
    bundledChangelogPromise = fetch(url, {
      ...
      if (parsed.length) return { entries: parsed, source: 'host' };
      return { entries: parseBundledChangelog(BUNDLED_CHANGELOG_FALLBACK), source: 'embedded' };
    }).catch(function () {
      return { entries: parseBundledChangelog(BUNDLED_CHANGELOG_FALLBACK), source: 'embedded' };
    });
  ...
  function openChangelogSheet() {
    ...
    toolSheet.appendChild(sourceMeta);
  ...
  function init(options) {
    ...
    fetch(cfg.configUrl, { headers: { 'Accept': 'application/json' }, cache: 'no-store' })
      ...
      .then(function (config) {
        ...
        if (typeof om.changelogUrl === 'string' && isRelativeSameOriginPath(om.changelogUrl)) {
          cfg.changelogUrl = om.changelogUrl;
        } else if (cfg.changelogUrl && !isRelativeSameOriginPath(cfg.changelogUrl)) {
          cfg.changelogUrl = null;
        }
