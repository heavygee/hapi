...
3. browser-hub has no `/api/stt`. Markup Send while listening must `finishRecording()`. HAPI web Quest hold-to-talk POSTs hub `POST /api/stt` (same transcribe path as Dictate) with the **logged-in HAPI JWT**. Gate secret stays on `/hapi/*`. Missing JWT is text-only, not a bad gate paste. Do not lasting-edit vendored dock for `sttAuth`.
