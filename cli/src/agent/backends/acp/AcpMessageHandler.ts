...
function needsInputEnrichment(
    ...
    if (!isUsableRawInput(existingInput)) return true;
...
export class AcpMessageHandler {
    ...
    private handleToolCall(update: Record<string, unknown>): void {
        ...
        // Empty `{}` is treated as missing (OpenCode tool-start / Cursor ACP /
        // permission clobber). Kimi streams args as JSON text in content.
        ...
        let input: unknown = isUsableRawInput(candidate) ? candidate : null;
        // Overlay locations path whenever derived input still lacks one
    ...
    private handleToolCallUpdate(update: Record<string, unknown>): void {
        ...
        if (isUsableRawInput(update.rawInput)) {
            ...
            // initial tool_call had neither usable rawInput nor a hoistable
            // thought. Never let empty `rawInput: {}` clobber a good input.
