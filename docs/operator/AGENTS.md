...
| Orphan `bun … hub/src/index.ts` pegging oos-linux CPU | **Never** hand-spawn test hubs (`bun -e` probes) — use `bun vitest run`. Cull: `hapi-cull-orphan-test-hubs.sh` (dry-run) / `--kill`. Upstream: Vitest `globalSetup` stamps `HAPI_TEST_MARKER` on the hub child | `scripts/tooling/hapi-cull-orphan-test-hubs.sh`; sibling: `context-mode-cull-spinners.sh` |
| systemd units drift / wrong KillMode / fleet PATH | **Do not** hand-edit `/etc/systemd/system/hapi-*` — install from repo: `install-hapi-systemd-units.sh` (`primary-soup` / `fleet-binary` / `user-pet`), then `verify-hapi-systemd-units.sh`. Lockhouse bootstrap must delegate | [`systemd-install.md`](../tooling/systemd-install.md) |
