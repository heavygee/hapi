...
| ⚠️ `needs_work` + `blockedUpstream` | `status:blocked-upstream` names a known external dependency; visible but not actionable by this peer | **never ping**; dependency terminal state is the wake condition |
