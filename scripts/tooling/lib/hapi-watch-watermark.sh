#!/usr/bin/env bash
# hapi-watch-watermark.sh — shared watermark / seen-set helpers for bare pollers.
#
# Strategies (config/watches.yaml state.strategy):
...
hapi_watch_ensure_parent() {
    local f
    f="$(hapi_watch_expand_path "$1")"
    mkdir -p "$(dirname "$f")"
}

# --- max-id -----------------------------------------------------------------

hapi_watch_max_id_read() {
    local f
    f="$(hapi_watch_expand_path "$1")"
    if [[ ! -f "$f" ]]; then
        echo 0
...
hapi_watch_seen_add() {
    ...
    f="$(hapi_watch_expand_path "$1")"
    id="${2:?id required}"
    hapi_watch_seen_ensure "$f"
    if hapi_watch_seen_has "$f" "$id"; then
        return 0
    fi
    printf '%s\n' "$id" >> "$f"
}
...
hapi_watch_timestamp_ids_write() {
    local f ts ids
    f="$(hapi_watch_expand_path "$1")"
...
hapi_watch_timestamp_ids_diff() {
    local state_file events_file
    state_file="$(hapi_watch_expand_path "$1")"
    events_file="${2:?events json file required}"
    if [[ ! -f "$state_file" ]]; then
        hapi_watch_timestamp_ids_seed "$state_file" 0
    fi
    jq -n \
