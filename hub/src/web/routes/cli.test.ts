...
describe('POST /cli/sessions/:id/session-rpc-capability', () => {
    const sessionId = '6212dae5-8a60-4284-b7a5-c09aa3571ce4'

    it('mints capability when machine tag matches the session owner machine', async () => {
        const app = createApp({
                ...
                id === sessionId
                    ? {
                        ...
                        session: {
                            id: sessionId,
                            active: false,
                            metadata: { machineId: 'machine-1', name: 'Owned' },
                        },
                    }
                    : { ok: false as const, reason: 'not-found' as const }
            ),
            getMachineByNamespace: (id: string, namespace: string) => (
                id === 'machine-1' && namespace === 'default'
                    ? { id: 'machine-1', namespace: 'default' }
                    : null
            ),
            getMachineTagByNamespace: (id: string) => (id === 'machine-1' ? 'secret-tag' : null),
            getMachine: () => null,
        } as never)

        const response = await app.request(`/cli/sessions/${sessionId}/session-rpc-capability`, {
            method: 'POST',
            headers: { ...authHeaders(), 'Content-Type': 'application/json' },
            body: JSON.stringify({ machineId: 'machine-1', machineTag: 'secret-tag' }),
        })
        expect(response.status).toBe(200)
    ...
    it('rejects when machine tag is wrong', async () => {
        const app = createApp({
            resolveSessionAccess: (id: string) => (
                id === sessionId
                    ? {
            ...
            getMachineByNamespace: () => ({ id: 'machine-1', namespace: 'default' }),
            getMachineTagByNamespace: () => 'secret-tag',
            getMachine: () => null,
        } as never)
        ...
        const response = await app.request(`/cli/sessions/${sessionId}/session-rpc-capability`, {
            method: 'POST',
            headers: { ...authHeaders(), 'Content-Type': 'application/json' },
            body: JSON.stringify({ machineId: 'machine-1', machineTag: 'wrong' }),
        })
        expect(response.status).toBe(403)
    })
})
