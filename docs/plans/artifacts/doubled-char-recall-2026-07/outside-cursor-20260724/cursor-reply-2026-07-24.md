...
Hey Mohit — appreciate the detailed reply. You're right on several points, and I want to be fair about them before I push back:

- **Non-determinism** (same session correct-then-wrong) and **"output exactly X" passing** are both consistent with sampling/recall. Agreed.
...
But the core of your argument — *"it happens across Auto/Composer/Opus/GPT alike, and a Cursor-side bug wouldn't vary by model, therefore it's the model, not Cursor"* — doesn't hold, and I now have the control experiment to show why.

## The logic gap

"Varies across models ⇒ not Cursor" only rules out a bug in a **model-specific** code path (e.g. a Composer-only bug). It does **not** rule out a factor in the **shared layer that sits in front of every model you route** — context assembly, the agent/system scaffolding, and your default sampling/effort settings. Those apply uniformly to Auto/Composer/Opus/GPT, so they'd produce exactly the "model-agnostic" pattern you're pointing at. Cross-model variance is evidence against a *per-model* bug — not evidence against a *Cursor-layer* cause.

The only test that separates "the model does this regardless" from "Cursor's shared layer induces it" is running **the same models outside Cursor**. That was the missing control. I ran it.

## The data (same models, driven directly — no Cursor)
...
**Over 125 emissions of `tiann` / `oos-linux` / a doubled-digit hostname, zero drops.** The distance-recall arm is the important one: it reproduces the exact situation your agent is in — the identifier lives earlier in the session (rules/history) and the model regenerates it later in prose under load. Verbatim, unedited receipts (fabricated host so I can share):
...
Strawberry is a **counting** task (how many r's) — a genuinely hard character-level operation. **Reproducing a string you were just given is not that** — and your own observation that "output exactly X" passes proves the model represents these characters fine. If this were the strawberry tokenization weakness, exact-copy would fail too. It doesn't. So the failure isn't "the model can't handle doubled characters"; it's "under some condition, the string the model regenerates isn't faithful to the one it was given." That condition is what differs between native and Cursor.

## Your mitigations are actually clues

- **"Keep identifiers in a rule/AGENTS.md so they sit where the model generates."** We do — and our distance-recall test literally seeds the string in-context and asks the model to regenerate it. Native reproduces it 100/100. So "the string is in context" is **not** sufficient to cause the drop natively. What happens to that string inside Cursor's context pipeline before the model generates from it?
...
## Where I'd look (given the model is constant)
...
Run this exact distance-recall protocol inside Cursor (Composer, Auto, Claude, your Codex route), N≥8 each, and share your per-emission drop rate. I'll send the harness and full data. If you come back at ~0 too, I'll happily close this. If you don't, the gap is in one of the four places above — and I'd genuinely like to fix the operator experience with you rather than argue about it.
