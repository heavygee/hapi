...
| Chip **🛑** `needs_operator` / `babysit.hold` | **Stop.** Do not thin, "address Tiann", or push. Operator ack only (`hapi-hold-ack` / `hapi hold-ack`). Never hourly-ping the coding peer. | § Meta PR watcher; [`2026-08-11-operator-hold-chip.md`](../plans/2026-08-11-operator-hold-chip.md) Track A |
...
| ⚠️ `needs_work` | failing CI, **current** open threads, bot findings, rebase, **closed-unmerged**, or `status:blocked-upstream` (#127) | fix per action string — **except** blocked-upstream: stay visible, `stickyPing:false` (#128) |
