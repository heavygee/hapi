...
#   1. Discovers the union of: open PRs on monitored repos (from project registry), 
#      recently-merged tracked PRs, and every hub session with a linked github_pr 
#      chip on monitored repos. Session TITLES ARE IGNORED for routing
...
#   5. Reads GitHub notifications for monitored repos since stored cursors
#      and folds new human comms into the action queue. Never marks read.
...
source "$SCRIPT_DIR/lib/project-registry.sh"

# Load project configuration
pr_load_config

# Backwards compatibility for existing env vars
UPSTREAM_REPO="${HAPI_PR_REPO:-tiann/hapi}"
FORK_REPO="${HAPI_FORK_REPO:-heavygee/hapi}"

# Discover all repos from project registry
get_all_monitored_repos() {
    local repos=()
    
    # If legacy env vars are set, use those first for backwards compatibility
    if [[ -n "${HAPI_PR_REPO:-}" || -n "${HAPI_FORK_REPO:-}" ]]; then
        [[ -n "${HAPI_PR_REPO:-}" ]] && repos+=("$HAPI_PR_REPO")
        [[ -n "${HAPI_FORK_REPO:-}" ]] && repos+=("$HAPI_FORK_REPO")
    else
...
md_state_default() {
    # Build notification cursor object for all monitored repos
    local cursor_obj="{}"
    local monitored_repos
    monitored_repos=$(get_all_monitored_repos)
    
    while IFS= read -r repo; do
        [[ -n "$repo" ]] && cursor_obj="$(printf '%s' "$cursor_obj" | jq -c --arg r "$repo" '.[$r] = null')"
    done <<< "$monitored_repos"
    
    jq -cn --argjson cursors "$cursor_obj" \
        '{schema:1,last_run:null,notif_cursor:$cursors,sessions:{},orphan_prs:{},notif_seen:{},hold:{},wave:{status:"idle",id:"w-empty",members:[],collect_started_at:null,collect_deadline_at:null}}'
}
...
# md_resolve_pr_home <number> → "repo\turl" or empty if no monitored repo has it.
# Tries all monitored repos in order. Never guess repo from the number alone.
#
...
md_resolve_pr_home() {
    ...
    while IFS= read -r repo; do
        ...
        url=""
        rc=0
        # Use project-registry-aware gh command
        local host
        host=$(pr_get_repo_host "$repo")
        if [[ "$host" != "github.com" ]]; then
            url="$("$GH_BIN" --hostname "$host" api "repos/${repo}/pulls/${number}" --jq '.html_url // empty' 2>/dev/null)" || rc=$?
        else
            url="$("$GH_BIN" api "repos/${repo}/pulls/${number}" --jq '.html_url // empty' 2>/dev/null)" || rc=$?
        fi
        
        if [[ "$rc" -eq 0 && -n "$url" && "$url" =~ ^https://[^/]+/.*pull/[0-9]+$ ]]; then
            # Return actual URL from API response
            printf '%s\t%s\n' "$repo" "$url"
            return 0
        fi
    done <<< "$monitored_repos"
...
md_backfill_refs() {
    ...
    local monitored_repos
    monitored_repos=$(get_all_monitored_repos | tr '\n' ' ')
    echo "Resolve order: $monitored_repos. Skip sessions that already have refs."
    echo ""
    ...
    while IFS=$'\t' read -r sid active name existing; do
        ...
        if [[ -z "$home" ]]; then
            PLAN_MISS+=("${sid:0:8}  PR #${first_pr} not on monitored repos: $monitored_repos  ($name)")
            continue
        fi
...
main() {
    ...
    # session iff metadata.externalRefs has github_pr on monitored repos.
    ...
    while IFS= read -r row; do
        ...
        local hapi_refs=""
        # Build dynamic repo filter for session chip filtering
        local repo_filter="false"
        local monitored_repos
        monitored_repos=$(get_all_monitored_repos)
        while IFS= read -r repo; do
            [[ -n "$repo" ]] && repo_filter="$repo_filter or (.repo // \"\") == \"$repo\""
        done <<< "$monitored_repos"
    ...
    # Update notification cursors for all monitored repos
    local cursor_updates=".last_run=\"$now_iso\""
    local monitored_repos
    monitored_repos=$(get_all_monitored_repos)
    
    # For now, keep backwards compatibility with UPSTREAM_REPO/FORK_REPO cursors
    cursor_updates="$cursor_updates | .notif_cursor[\"$UPSTREAM_REPO\"]=\"$cursor_up\""
    cursor_updates="$cursor_updates | .notif_cursor[\"$FORK_REPO\"]=\"$cursor_fork\""
    
    # TODO: Add proper per-repo notification handling for other repos
    
    new_state="$(printf '%s' "$new_state" | jq -c "$cursor_updates")"
...
_do_ping() {  # <sid8> <emoji> <prs> <acts>
    ...
    if [[ -n "$sess_path" ]] && ! pec_path_is_hapi_estate "$sess_path"; then
        ...
        # Check if session has monitored repo chips using same dynamic filter
        local repo_filter="false"
        local monitored_repos
        monitored_repos=$(get_all_monitored_repos)
        while IFS= read -r repo; do
...
_print_queue() {
    ...
    echo ""
    echo "NEXT STEPS:"
    echo "  - Lanes A/B apply ONLY to tiann/hapi. heavygee/hapi is operator/Meta choice"
    echo "    (no size caps / low-impact ritual / wait-on-tiann)."
    ...
    echo "  - Lane B (SELF-MERGE, upstream): tests/docs auto or low-impact/allowlist."
