export default {
  ...
  'settings.display.pinInProgressSessions.desc': '侧边栏顶部「进行中」分区的置顶程度。默认为长时间任务（带会话 job 进度的后台批处理）。安静的活跃智能体仍留在项目目录中，除非选择「全部活动」。全局置顶以及含项目置顶的项目分组仍在该分区之上；关闭后全部保留在目录分组中。',
