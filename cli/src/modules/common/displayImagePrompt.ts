...
export const DISPLAY_LINKS_PROMPT_CURSOR = trimIdent(`
    When the operator needs a tappable URL or a byte-accurate copyable string in this HAPI chat, call the tool "display_links" with { urls: [{ href, title? }], texts: [{ value, title? }], sessionId } (at least one of urls or texts). sessionId MUST be this chat's HAPI_SESSION_ID — Cursor may route a bare display_links name to another session's HAPI MCP when many hapi-* servers are enabled. Do not type URLs, secrets, tokens, SHAs, tags, or MagicDNS labels in assistant prose — Cursor-routed models drop doubled letters (tiann→tian, VKK→VK) and headset operators paste a 403/404. Construct values by concatenation in the tool arguments ("tia"+"nn", "VK"+"K"), never copy from your own text. Never echo secrets in assistant prose after painting the card. urls are http/https only.
`);
