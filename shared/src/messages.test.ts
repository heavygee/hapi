...
describe('matchNotifySummaryLine (corruption-tolerant detector)', () => {
    test('matches the correct token line and returns the JSON', () => {
        expect(matchNotifySummaryLine('AGENT_NOTIFY_SUMMARY {"status":"done"}')?.jsonPart).toBe('{"status":"done"}')
    })

    test('matches the Cursor dup-dropped token (SUMARY)', () => {
        expect(matchNotifySummaryLine('AGENT_NOTIFY_SUMARY {"status":"done"}')?.jsonPart).toBe('{"status":"done"}')
    })

    test('tolerates trailing/leading whitespace on the line', () => {
        expect(matchNotifySummaryLine('  AGENT_NOTIFY_SUMMARY {"a":1}  ')?.jsonPart).toBe('{"a":1}')
    })

    test('allows optional prose prefix before the token on the same line', () => {
        // Glued footers only — whitespace-delimited mentions are not footers (#1477).
        expect(matchNotifySummaryLine('see the AGENT_NOTIFY_SUMMARY {"x":1}')).toBeNull()
        expect(matchNotifySummaryLine('Ownership session pinged.AGENT_NOTIFY_SUMMARY {"version":1,"status":"done","summary":"ok"}')?.jsonPart)
            .toBe('{"version":1,"status":"done","summary":"ok"}')
    })

    test('allows glued prose before a corrupted (SUMARY) token', () => {
        expect(matchNotifySummaryLine('pinged.AGENT_NOTIFY_SUMARY {"status":"done"}')?.jsonPart).toBe('{"status":"done"}')
    })
