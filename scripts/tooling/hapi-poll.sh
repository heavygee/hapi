#!/usr/bin/env bash
# hapi-poll — declarative registry + systemd generator for bare (zero-token) pollers.
#
# Canon: docs/plans/2026-09-18-scheduled-agent-tasks-design.md
# Runbook: docs/tooling/hapi-poll.md
#
...
#   hapi poll templates
#
# Also: hapi-poll <subcommand> …  (PATH wrapper)

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$(readlink -f "$0")")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
REGISTRY="${HAPI_POLLS_YAML:-$REPO_ROOT/config/polls.yaml}"
TEMPLATES_DIR="$SCRIPT_DIR/poll-templates"
UNIT_OUT_DIR="$SCRIPT_DIR/systemd/polls"
WATERMARK_LIB="$SCRIPT_DIR/lib/hapi-poll-watermark.sh"

# shellcheck source=lib/hapi-poll-watermark.sh
source "$WATERMARK_LIB"

err() { echo "hapi-poll: $*" >&2; }
die() { err "$*"; exit 2; }
...
# Emit JSON for one poll (or all) via python+yaml.
registry_json() {
    local name="${1:-}"
...
try:
    ...
    sys.stderr.write("hapi-poll: PyYAML required (python3 -c 'import yaml')\n")
    sys.exit(3)
with open(path) as f:
    doc = yaml.safe_load(f) or {}
polls = doc.get("polls") or []
if name:
    matches = [w for w in polls if w.get("name") == name]
    if not matches:
        sys.stderr.write(f"hapi-poll: unknown poll '{name}' in {path}\n")
        sys.exit(1)
    json.dump(matches[0], sys.stdout)
else:
    json.dump({"version": doc.get("version", 1), "polls": polls}, sys.stdout)
PY
}
...
validate_one() {
    ...
    script="$(poll_field "$json" '.probe.script')"
    strategy="$(poll_field "$json" '.state.strategy')"
    state_path="$(expand_user_path "$(poll_field "$json" '.state.path')")"
    host="$(poll_field "$json" '.host')"
    ...
    echo "hapi-poll: validate ok — $name (warnings=$warnings)"
    return 0
}
...
cmd_validate() {
    ...
    local names
    names="$(registry_json | jq -r '.polls[].name')"
    local n
    for n in $names; do
        validate_one "$n"
    done
...
generate_units() {
    ...
    script="$(poll_field "$json" '.probe.script')"
    calendar="$(calendar_from_cadence "$(poll_field "$json" '.cadence')")"
    delay="$(poll_field "$json" '.randomized_delay_sec' '90')"
    user="$(poll_field "$json" '.user' "${USER:-heavygee}")"
    workdir="$(poll_field "$json" '.working_directory' "/home/$user")"
    lock="$(expand_user_path "$(poll_field "$json" '.lock.path' "")")"
    if [[ -z "$lock" ]]; then
        lock="/home/$user/.local/state/hapi/poll-${name}.lock"
    fi
    state_path="$(expand_user_path "$(poll_field "$json" '.state.path')")"
    desc="$(poll_field "$json" '.description' "HAPI poll: $name")"
    # Prefer the primary-mirror registry path in unit Documentation= when generating
    # from a worktree (install paths in polls.yaml already point at runtime scripts).
    doc_registry="$REGISTRY"
    if [[ "$REPO_ROOT" == */worktrees/* ]]; then
        doc_registry="${HAPI_MIRROR_ROOT:-$HOME/coding/hapi}/config/polls.yaml"
    fi
    ...
    {
        echo "# Generated by hapi-poll from $REGISTRY"
        echo "# Do not hand-edit; re-run: hapi poll install $name"
        echo
        echo "[Unit]"
...
cmd_install() {
    ...
    [[ -n "$name" ]] || die "usage: hapi poll install <name> [--force] [--run-now] [--dry-run]"
    ...
    echo "hapi-poll: generated:"
    echo "$files" | sed 's/^/  /'

    if [[ "$dry" -eq 1 ]]; then
        echo "hapi-poll: dry-run — not installing to /etc/systemd/system"
        return 0
    fi
    ...
    echo "hapi-poll: installed ${prefix}.timer"
    systemctl list-timers "${prefix}*" --all --no-pager || true

    if [[ "$run_now" -eq 1 ]]; then
        echo "hapi-poll: starting one tick now…"
...
cmd_uninstall() {
    local name="${1:-}"
    [[ -n "$name" ]] || die "usage: hapi poll uninstall <name>"
    local prefix
    prefix="$(unit_prefix "$name")"
    if [[ "$(id -u)" -ne 0 ]]; then
    ...
    systemctl daemon-reload
    echo "hapi-poll: uninstalled $prefix"
}

cmd_run() {
    local name="${1:-}"
    [[ -n "$name" ]] || die "usage: hapi poll run <name>"
    validate_one "$name" >/dev/null
    ...
    lock="$(expand_user_path "$(poll_field "$json" '.lock.path' "")")"
    if [[ -z "$lock" ]]; then
        lock="$(expand_user_path ~/.local/state/hapi/poll-${name}.lock)"
    fi
    mkdir -p "$(dirname "$lock")"
    echo "hapi-poll: running $script (flock $lock)"
...
doctor_one() {
    ...
    if [[ -f "$state_path" ]]; then
        case "$strategy" in
            max-id)
                echo "  lastMaxId=$(hapi_poll_max_id_read "$state_path")  mtime=$(date -u -r "$state_path" +%FT%TZ 2>/dev/null || stat -c %y "$state_path")"
                ;;
            seen-set)
                echo "  seen_count=$(wc -l < "$state_path" | tr -d ' ')  mtime=$(date -u -r "$state_path" +%FT%TZ 2>/dev/null || true)"
                ;;
            timestamp-ids)
                echo "  $(hapi_poll_timestamp_ids_read "$state_path")"
                ;;
            *)
...
cmd_doctor() {
    ...
    local n
    for n in $(registry_json | jq -r '.polls[].name'); do
        doctor_one "$n" || rc=1
        echo
    done
    return $rc
