/*
 * Visual proof for tiann/hapi#1431:
 * With pin-in-progress ON, a project-pin group renders above In progress,
 * which still beats unpinned directory groups.
 *
 * Run from mirror:
 *   cd ~/coding/hapi && node scripts/dev/run-e2e-on-peer-stack.mjs \
 *     --worktree ~/coding/hapi/worktrees/project-pin-above-in-progress \
 *     --name project-pin-above-in-progress --keep \
 *     e2e/peer/1431-project-pin-above-in-progress.spec.ts
 */
...
const PNG_ORDER = resolve(OUT, '1431-project-pin-above-in-progress.png')

const runId = `${Date.now()}`
const PROJECT_PIN_TITLE = `Proof1431 ProjectPin ${runId}`
const FLOATER_TITLE = `Proof1431 Floater ${runId}`
const OTHER_IDLE_TITLE = `Proof1431 OtherIdle ${runId}`
const PROJECT_PIN_PATH = `/tmp/proof1431-pinned-${runId}`
const OTHER_PATH = `/tmp/proof1431-other-${runId}`
...
async function createSession(title: string, path: string): Promise<string> {
    const res = await fetch(`${hubUrl}/cli/sessions`, {
        ...
        body: JSON.stringify({
            tag: `proof1431-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
            metadata: {
                path,
                host: hostname(),
                flavor: 'cursor',
...
test.describe('1431 project pin above In progress', () => {
    test('project-pin group precedes In progress which precedes other groups', async ({ page }) => {
        requirePeerEnv()
        ...
        try {
            ...
            await expect(page.getByRole('button', { name: new RegExp(PROJECT_PIN_TITLE) }).first()).toBeVisible()
            ...
            if (!pinBox || !runningBox || !otherBox) {
                throw new Error('missing section bounding boxes')
            }
            expect(pinBox.y).toBeLessThan(runningBox.y)
            expect(runningBox.y).toBeLessThan(otherBox.y)

            await projectPinGroup.scrollIntoViewIfNeeded()
