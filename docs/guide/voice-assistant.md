...
Realtime OpenAI, ElevenLabs, and Deepgram dictation sessions receive only short-lived credentials minted by the hub. Gemini Live and Qwen Realtime assistant sessions connect through hub-side WebSocket proxies, so those API keys never reach the browser either. Eligible desktop browsers with the on-device `SpeechRecognition` API expose **Browser on-device** as a realtime-only dictation provider. HAPI checks the selected language pack when dictation starts and never falls back from that option to browser-hosted recognition. Mobile and unknown browser environments fail closed for this provider specifically: some Android WebViews expose a partial, crash-prone implementation, and separately, no mobile browser today ships bundled on-device speech models, so real mobile Chrome would only ever report the feature unavailable anyway. The four server-backed dictation providers above (OpenAI, ElevenLabs, Deepgram, Groq/OpenAI-compatible) work identically on mobile and desktop — this restriction applies only to the free/offline on-device option.

Browsers with the classic Web Speech API (`SpeechRecognition`/`webkitSpeechRecognition`, no on-device extension) additionally expose **Browser (cloud speech)** as a realtime-only dictation provider — this is the mobile-capable counterpart to Browser on-device: it works on Chrome for Android and Safari on iOS, since it uses the browser vendor's own cloud speech service (Google or Apple) rather than a bundled on-device model. It requires no API key and no HAPI-side credential, but audio for this option goes to Google's or Apple's servers, not to a HAPI-configured provider — pick it deliberately, the same way you'd pick any other cloud provider. Firefox does not implement this API and the option is hidden there. Both browser-based providers can appear together and are independently selectable when supported.
