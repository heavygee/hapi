...
| Proof / screenshots / clips for operator | **Inline into HAPI chat** — do not only paste paths | **Estate default:** [`operator-visible-proof`](../../../skills/operator-visible-proof/SKILL.md) skill. HAPI deep dive: lifecycle [§ Proof tiers](../tooling/feature-work-lifecycle.md#proof-tiers-images-and-video) — MCP `display_image` / `display_video`, or `bun scripts/tooling/hapi-display-image.mjs <session-prefix> <abs-path> [title]` |
