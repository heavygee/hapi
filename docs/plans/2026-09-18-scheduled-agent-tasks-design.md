...
**Status:** design signed off; v1 as **`hapi tick`** (final — rejected `watch` for Wear/watch app collision and `poll`/`monitor` ambiguity; operator confirmed). See `docs/tooling/hapi-tick.md`, issue #153.
**Date:** 2026-09-18  
...
**Proposal:** v1 documents the gap; v1.5 adds `until:` / `max_ticks` / `exit_when` on poll spec, or a **`hapi tick run --until <expr>`** wrapper for session-scoped babysits (still durable unit, but probe script calls `systemctl disable` on success). Do not conflate this with mechanical standing pollers.
...
A **Tick** (or **poller** where clearer) is a declarative record:
...
## Creating a tick without hand-writing systemd
...
1. **Name:** **Decided (final): `hapi tick`.** No further rename.
