...
**2026-09-03 addendum — operator decision for the live case (Doug/Antevorta):** the situation moved into the §4 exception. Doug's Antevorta is a genuinely independent hub, not a shared-hub client — but the operator picked that path for **organizational**, not technical, reasons that this doc didn't originally weigh: (1) Doug's installation is out-of-scope for this estate and an in-scope install is a separate future move; (2) more decisively, the operator does not want to be "a single point of failure for [Doug's] stuff being up for them doing work." That's an availability/blast-radius argument this doc's §4 didn't cover — §4 asked "does this workmate need independent-hub capability," not "should the operator's uptime be a dependency of someone else's work." Given that framing, Option A (local rebuild) was the deliberate choice, and its cost/risk from §3 was tested for real rather than left theoretical.

**Result of that rehearsal:** [`2026-09-03-soup-packaging-vm-rehearsal.md`](./2026-09-03-soup-packaging-vm-rehearsal.md) — **Option A as "clone + rebuild the recipe" does not work.** Tip-forward remat from a fresh host does not reproduce the dogfood tip (cold clone failed on layer 1/48 with a merge conflict tip-forward on oos never sees, because oos starts from an already-healed green tip, not bare upstream). A **frozen snapshot** of the composed `driver/integration` tree (git bundle/mirror at a known-good SHA, proven bit-identical tree OID in the rehearsal) is the distribution unit that actually works. So: independent hub — yes, per the operator's organizational call above; "hand Doug the manifest and let him rebuild" — no, ship him a tip, not a recipe.
