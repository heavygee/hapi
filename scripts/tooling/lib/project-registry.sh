...
pr_load_config() {
    ...
    # Mark as loaded
    PR_CONFIG_LOADED["$config_path"]=1
    ...
    for project in $projects; do
        local targets
        targets=$(yq eval ".projects.$project.targets[]" "$config_path" 2>/dev/null || echo "")
        
        # Parse each target
        while IFS= read -r target_yaml; do
            if [[ -n "$target_yaml" ]]; then
                local repo host
                repo=$(echo "$target_yaml" | yq eval '.repo' -)
                host=$(echo "$target_yaml" | yq eval '.host // "github.com"' -)
                ...
                if [[ -n "$repo" && "$repo" != "null" ]]; then
                    ...
                    local target_id control primary
                    target_id=$(echo "$target_yaml" | yq eval '.id' -)
                    control=$(echo "$target_yaml" | yq eval '.control' -)
                    primary=$(echo "$target_yaml" | yq eval '.primary // false' -)
        ...
        done <<< "$(yq eval ".projects.$project.targets[]" "$config_path")"
    done
}
...
pr_get_project_for_repo() {
    ...
    if [[ -z "${PR_CONFIG_LOADED[$DEFAULT_PROJECTS_CONFIG]:-}" ]]; then
        pr_load_config
    fi
...
pr_get_target_for_repo() {
    local repo="$1"
    local project
    project=$(pr_get_project_for_repo "$repo")
    
    if [[ -z "$project" ]]; then
        # Default to upstream/theirs for unknown repos
        echo "upstream	theirs	github.com	true"
        return 0
    fi
    
    # Find the matching target for this repo
    local targets
    targets=$(yq eval ".projects.$project.targets[] | select(.repo == \"$repo\")" "$DEFAULT_PROJECTS_CONFIG" 2>/dev/null || echo "")
    
    if [[ -n "$targets" ]]; then
        local target_id control host primary
        target_id=$(echo "$targets" | yq eval '.id' -)
        control=$(echo "$targets" | yq eval '.control' -)
        host=$(echo "$targets" | yq eval '.host // "github.com"' -)
        primary=$(echo "$targets" | yq eval '.primary // false' -)
        
        echo "$target_id	$control	$host	$primary"
    else
        # Default fallback
        echo "upstream	theirs	github.com	true"
    fi
