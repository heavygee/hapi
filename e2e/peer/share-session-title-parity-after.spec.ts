/*
 * AFTER-fix peer-stack proof for tiann/hapi#1218 — /share titles match sidebar.
 * Fork main only. Run with --no-up after rebuilding peer web dist.
 */
...
const SIDEBAR_SHOT = resolve(artifactRoot, 'localdocs/playwright-runs/share-title-parity-after-sidebar.png')
const SHARE_SHOT = resolve(artifactRoot, 'localdocs/playwright-runs/share-title-parity-after-share.png')
...
function seedMismatchedSession(): void {
...
async function seedShareTransfer(page: Page, transferId: string): Promise<void> {
    await page.evaluate(async ({ id }) => {
        const payload = {
            title: 'Title parity after-fix',
            text: 'Proof payload for share picker title parity',
            url: '',
...
test.describe('share session title parity — AFTER fix (#1218)', () => {
    test.beforeAll(() => {
        requirePeerEnv()
    })

    test('sidebar and /share both show metadata.name', async ({ page }) => {
        ...
        const transferId = `share-title-after-${Date.now()}`
        ...
        await expect(page.getByRole('button', { name: new RegExp(SIDEBAR_NAME) }).first()).toBeVisible({
            timeout: 30_000,
        })
        // Fixed: share picker must not prefer the divergent summary
        await expect(page.getByRole('button', { name: new RegExp(SUMMARY_TEXT) })).toHaveCount(0)
