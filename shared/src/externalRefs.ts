...
import { isValidGitHubHost, getGitHubUrlForRepo } from './projectRegistry'
...
export function githubPrUrl(repo: string, number: number, host = "github.com"): string {
...
export function parseGithubPrInput(raw: string): ParseGithubPrInputResult {
    ...
    if (url.protocol !== 'https:' || !isValidGitHubHost(url.hostname)) {
        return { ok: false, error: 'expected https://<valid-host>/.../pull/N URL' }
    }
