...
export function SessionList(props: {
    ...
    const blockedSectionSessions = useMemo(() => {
        const now = Date.now()
        return machineFilteredSessions
            .filter(session => !session.globalPinned && sessionIsBlocked(session, { now }))
            .sort((a, b) => b.updatedAt - a.updatedAt)
    }, [machineFilteredSessions])
    ...
    const runningSessions = useMemo(() => {
        ...
        return buckets
    }, [machineFilteredSessions, pinInProgressSessions, blockedSectionIds])
    ...
    const actionOnlyGroups = useMemo(() => {
        ...
        if (!pinInProgressSessions && blockedSectionIds.size === 0) {
            return []
        }
        const visibleKeys = new Set(groups.map((group) => group.key))
        return allDirectoryGroups.filter((group) => !visibleKeys.has(group.key))
    }, [groups, allDirectoryGroups, pinInProgressSessions, blockedSectionIds, showBlockedOnly])
    ...
    return (
        <div className="flex min-h-0 w-full flex-1 flex-col">
            ...
            <div className="mx-auto flex w-full max-w-content flex-col gap-1 pl-1.5 pr-2 pb-2">
                ...
                {props.sessions.length > 0 && (isFiltering || activeMachineFilter !== null || showUnreadOnly || showBlockedOnly) && groups.length === 0 && runningSessionTotal === 0 && activeSessionTotal === 0 && globalPinnedSessions.length === 0 && blockedSectionSessions.length === 0 ? (
