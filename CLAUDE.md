...
- **Session titles = workstream only.** Never put ✅/🔁/⚠️/📝/🔧 in the title for PR health (ADR D8). Chip owns status (`hapi-meta-daily.sh`); deprecated `hapi-pr-session-emoji.sh` must not be worked around with a manual rename. See [`feature-work-lifecycle.md` § Session titles and PR chips](docs/tooling/feature-work-lifecycle.md#session-titles-and-pr-chips-no-status-emoji).
