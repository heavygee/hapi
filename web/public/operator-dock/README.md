...
Pinned tag: **v0.12.0**  
Source: https://github.com/heavygee/hapi-inline/releases/tag/v0.12.0
...
- `operator-dock.js` — `384431e953b2358a59973adee618103368200d5a`
- `operator-dock.css` — `a886eb03a6fefed579e1591a6c7f9fb8565d1834`
...
- `hub/src/web/hapi-inline/` — operator-gated `/hapi` proxy. `PINNED_TAG` in `config.ts` is what the mic payload `build` field reports (must match this README pin). Cursor spawn sends `model: auto`. Allow-list: POST `messages` / `upload` / `abort` (GET abort forbidden). `web/index.html` loads dock assets with `?v=<pin>`.
