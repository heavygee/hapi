...
- **NEVER merge on `tiann/hapi` from an agent shell.** Agents prepare only. Lanes A/B/C apply **only to upstream** (`tiann/hapi`): Lane A = @tiann; lane B = operator/Meta for policy-eligible low-impact PRs (blessing: #1268); lane C = forbidden. They do **not** constrain `heavygee/hapi` — fork merges are operator/Meta choice. See `docs/operator/AGENTS.md` § Upstream relationship. Even with write access, even when a handoff says "babysit to merge" - prepare only on upstream unless an operator with a TTY explicitly directs a lane B merge.
- **Session titles = workstream only.** Never put ✅/🔁/⚠️/📝/🔧 in the title for PR health (ADR D8). Chip owns status (`hapi-meta-daily.sh`); the `hapi-pr-session-emoji.sh` stub only prints that successor — do not work around with a manual rename. See [`feature-work-lifecycle.md` § Session titles and PR chips](docs/tooling/feature-work-lifecycle.md#session-titles-and-pr-chips-no-status-emoji).
